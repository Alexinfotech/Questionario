...
// Recupera solo le domande vero/falso per il questionario specifico
$query_questions = "SELECT qd.id, d.testo_domanda
                    FROM questionario_domande qd
                    JOIN domande d ON qd.id_domanda = d.id
                    WHERE qd.id_questionario = ? AND d.tipo_domanda = 'vero_falso'";
...
$stmt_questions->bind_param("i", $questionario_id);
$stmt_questions->execute();
$result_questions = $stmt_questions->get_result();

if ($result_questions === false || $result_questions->num_rows == 0) {
    echo "<p>Non ci sono domande vero/falso disponibili per questo questionario.</p>";
    $conn->close();
    exit;
}
?>
...
<div class="container mt-5">
    ...
    <input type="hidden" name="questionario_id" value="<?php echo htmlspecialchars($questionario_id); ?>">
    <?php while ($row = $result_questions->fetch_assoc()) {
        ...
        echo "<input type='hidden' name='tipo_domanda_{$domandaId}' value='vero_falso'>";
    ...
    <button type="button" id="inviaRisposte" class="btn btn-primary">Invia Risposte</button>
</form>
...
document.getElementById("inviaRisposte").addEventListener("click", function() {
    ...
    // Invia le risposte vero/falso a invioRisposteVeroFalso.php
    fetch("php/invioRisposteVeroFalso.php", {
        method: "POST",
        body: formData
    }).then(response => {
        if (!response.ok) {
            throw new Error("Errore nell'invio delle risposte vero/falso");
        }
